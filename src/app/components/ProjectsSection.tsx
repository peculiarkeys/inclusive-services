...
// Dynamically import all images in the obembe_projects folder
...
const projectTypes = [
  ...
  { type: "Luxury Villa Construction", category: "residential" },
  { type: "Multi-Family Apartment Building", category: "residential" },
  { type: "Industrial Warehouse Construction", category: "structural" },
  ...
  { type: "Turnkey Residential Estate", category: "residential" },
  ...
  { type: "Interior Renovation & Fit-Out", category: "renovations" },
  { type: "MEP & Finishing Services", category: "renovations" },
];

const projects = sortedPaths.map((path, index) => {
  const img = rawImages[path];
  const location = "Nigeria";
  const typeObj = projectTypes[index % projectTypes.length];
  return {
    img,
    location,
    title: typeObj.type,
...
const categories = [
  ...
  { id: "renovations", label: "Renovations & Finishing" },
];
      ...
      <div className="max-w-[1320px] mx-auto">
        <AnimatedSection className="text-center mb-12">
          <h2 className="text-white mb-4" style={{ fontSize: "clamp(32px, 5vw, 64px)", fontWeight: 400, lineHeight: "1.2", letterSpacing: "-2px" }}>
            Our Exclusive Client's Project Sites
          </h2>
          <p className="text-white/60 max-w-2xl mx-auto" style={{ fontSize: "16px", lineHeight: "24px" }}>
            A curated showcase of our engineering excellence, procurement capability, and construction quality across key locations.
          </p>
        </AnimatedSection>
        ...
        {filteredProjects.length > visibleCount && (
          <AnimatedSection delay={0.3} className="flex justify-center">
            <button
              ...
              className="inline-flex items-center gap-2.5 text-white px-5 py-3 rounded-xl border border-white/20 hover:border-white/50 hover:bg-white/5 transition-all text-sm font-medium cursor-pointer"
            >
            ...
            >
              ...
              <div className="w-full text-center text-white px-4 mt-2">
                <span className="text-[#FF6501] text-xs font-semibold uppercase tracking-wider mb-1 block">
                  {categories.find((c) => c.id === filteredProjects[lightboxIndex].category)?.label}
                </span>
                <h3 className="text-lg md:text-2xl font-medium tracking-tight mb-1 text-white">
